...
class Compose:
    ...
    def __init__(self, transforms, to_rgb=False, img_channels=3, foreground=True):
    ...
    def __call__(self, data):
        ...
        if 'label' in data.keys() and isinstance(data['label'], str):
            ...
            if 'gt_fields' in data and 'mask' in data['gt_fields']:
                data['mask'] = np.where(data['label'] != 0, 1, 0)
                # data['label'] = np.where(np.isin(data['label'], [1, 5]), 1, 0)
        ...
        if 'gt_fields'in data and 'mask' in data['gt_fields']:
            ...
            # data['img'] = data['img'][..., np.newaxis]
            data['img'] = np.concatenate((data['img'], data['mask']), axis=2)
